  ...
  <main>
    ...
    <modal v-show="signupSuccessful">
      <div class="modal-popup">
        ...
        <button class="primary-button" @click="() => (verifyEmail = true)">
          Continue
        </button>
      </div>
    </modal>
    ...
    <modal v-show="verifyEmail">
      <div class="modal-popup">
        ...
        <button
          ...
          @click="() => (verifyEmailSuccessful = true)"
    ...
    <modal v-show="verifyEmailSuccessful">
      <div class="modal-popup">
        ...
        <router-link to="signin"
          ><button class="primary-button">Ok</button></router-link
        >
      </div>
    </modal>
    ...
    <div class="form-wrapper">
      ...
      <form @submit.prevent="signup">
        ...
        <div v-show="displayChecker" class="password-checker">
          <ul class="check-list">
            <li :class="checkNumber">At least 1 number</li>
            <li :class="checkUpperCase">At least 1 uppercase</li>
            <li :class="checkMinimum">At least 8 characters</li>
          </ul>
        </div>
...
export default {
  data() {
    return {
      ...
      checkNumber: "check number-error",
      checkUpperCase: "check upper-error",
      checkMinimum: "check min-error",
  ...
  methods: {
    ...
    passwordChecker() {
      this.displayChecker = true;
      this.numRegex = /\d/;
      this.upperRegex = /[A-Z]/;
      this.passwordLength = this.user.password.length;

      if (this.numRegex.test(this.user.password)) {
        this.checkNumber = "check number-pass";
      } else {
        this.checkNumber = "check number-error";
      }

      if (this.passwordLength >= 8) {
        this.checkMinimum = "check min-pass";
      } else {
        this.checkMinimum = "check min-error";
      }

      if (this.upperRegex.test(this.user.password)) {
        this.checkUpperCase = "check upper-pass";
      } else {
        this.checkUpperCase = "check upper-error";
      }
    },
  },
};
...
.check.min-error {
  color: #c12828;
  list-style-image: url(../../assets/icons/check-error.svg);
}
...
.check.min-pass {
  color: #449e3c;
  list-style-image: url(../../assets/icons/check-pass.svg);
}
